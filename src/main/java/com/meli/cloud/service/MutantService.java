...
public class MutantService {
	...
	public boolean isMutant(String[] dna) {
		...
		DnaMutation dnaMutation = DnaMutation.builder().dna(dna).mutations(mutations).build();

		String dnaSearch = dnaMutation.toString();
	...
	/**
	 * This Method is using bactraking for search the results that match the
	 ...
	 * @return true of false
	 */
	boolean validateMovement(char[][] dna, int missingMovements, int xPos, int yPos, int[] movement,
			char charToValidate) {
		...
		if (nextChar == charToValidate) {
			if (missingMovements == 1) {
				...
				return validateMovement(dna, missingMovements - 1, xPos + movement[0], yPos + movement[1], movement,
						charToValidate);
			}
		} else {
			return false;
	...
	/**
	 * This Method verify if the String[] have characters allowed and the length be
	 * correct
	 * 
	 * @param dna
	 * @return true of false
	...
	public boolean validateDNA(String[] dna) {
		...
		for (String line : dna) {
			...
			long discard = myCodes.stream().filter(x -> x.matches("[^ATCGatcg]+")).count();
